...
final class aahe extends SQLiteOpenHelper
{
    private final int a;
    
    public aahe(Context writableDatabase) {
        ...
        try {
            while (((Cursor)o).moveToNext()) {
                try {
                    new aade(((Cursor)o).getBlob(columnIndexOrThrow2));
                    new aade(((Cursor)o).getBlob(columnIndexOrThrow3));
                }
    ...
    public final void onUpgrade(final SQLiteDatabase sqLiteDatabase, int n, final int n2) {
                ...
                try {
                    ...
                    try {
                        while (query.moveToNext()) {
                            ...
                            final aade aade = new aade(query.getBlob(columnIndexOrThrow2));
                            if (aade.p("thumbnail", false)) {
                                aahc.T((aach)aade, 2);
                            }
                            else if (aade.p("ad", false)) {
                                aahc.T((aach)aade, 3);
                            }
                            else {
                                aahc.T((aach)aade, 1);
        ...
        tut.d(String.format(Locale.US, "[Offline] Error trying to upgrade from %d to %d. Wiping data and create database from scratch.", n, n2), (Throwable)query);
        tgs.f(sqLiteDatabase);
        this.onUpgrade(sqLiteDatabase, 0, this.a);
    }
}
