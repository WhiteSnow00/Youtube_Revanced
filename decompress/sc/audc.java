...
public abstract class audc extends ICronetEngineBuilder
{
    ...
    public audc(final Context context) {
        ...
        if (!audc.n.matcher(s).matches()) {
            if (s.length() <= 255) {
                try {
                    ...
                    this.c.add(new auda(ascii, ((Map<K, Object>)s).values().toArray(new byte[((Map)s).size()][]), b, date));
                    return;
                }
