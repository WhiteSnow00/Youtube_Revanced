...
public final class czq
{
    ...
    public static float a() {
        float f;
        if ((f = czq.f) == -1.0f) {
            f = (czq.f = Resources.getSystem().getDisplayMetrics().density);
        }
        return f;
    }
    ...
    public static void g(final Path path, final cvk cvk) {
        if (cvk != null) {
            if (!cvk.a) {
                h(path, ((cvs)cvk.b).k() / 100.0f, ((cvs)cvk.c).k() / 100.0f, ((cvs)cvk.d).k() / 360.0f);
            }
        }
    }
    
    public static void h(final Path path, float n, float n2, float n3) {
        final PathMeasure pathMeasure = czq.b.get();
        final Path path2 = czq.c.get();
        final Path path3 = czq.d.get();
        ...
        if (n2 >= length) {
            ...
            n = n5;
            if (n5 >= length) {
                n3 = (float)czl.d(n2, length);
                n = (float)czl.d(n5, length);
            }
        }
        n2 = n3;
        if (n3 < 0.0f) {
            n2 = (float)czl.d(n3, length);
        }
        n3 = n;
        if (n < 0.0f) {
            n3 = (float)czl.d(n, length);
        }
        if (n2 == n3) {
            path.reset();
            ctp.a();
